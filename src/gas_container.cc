...
GasContainer::GasContainer(int num_of_particles, int velocity, int radius, ci::Color color, const int size) {
  ...
  for (int i = 0; i < num_of_particles; i++) {
    ...
    particles_.push_back(Particle(vec2(kMargin * 4, kMargin * 4), vec2(velocity_x,velocity_y), radius, color));
  }
}
...
void GasContainer::Display() {
  ...
  ci::gl::drawStrokedRect(ci::Rectf(vec2(kMargin, kMargin), vec2(kSize, kSize)));
}
